...
const userController = {
        ...
        User.findOne({ _id: params.id })
            .then(dbUserData => {
                ...
                Promise.all([dbThoughtData, deleteUserData])
                    .then(dbData => {
                        if (!dbData) {
                            res.status(404).json({ message: 'Something went wrong. Please check' });
                            return;
                        }
                        res.json(dbData);
                    })
    ...
    addNewFriend({ params }, res) {
        User.findOne({ _id: params.friendId })
            .then(dbFriendData => {
                if (!dbFriendData) {
                    res.status(404).json({ message: 'No user found with this ID! You cannot add to your friend list' });
                    return;
                }
